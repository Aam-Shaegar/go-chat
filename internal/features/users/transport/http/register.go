...
type RegisterResponse domain_dtos.AuthResponseDTO
...
func (h *UsersHTTPHandler) Register(w http.ResponseWriter, r *http.Request) {
	...
	authResp, refreshToken, err := h.usersService.Register(ctx, req.Username, req.Email, req.Password)
	if err != nil {
	...
	response := RegisterResponse(authResp)
